#!/usr/bin/env tsx
/**
 * Drift detection for Ivory Coast Law MCP.
 *
 * Checks if upstream Ivory Coast Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Ivory Coast Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'ivorian-law-mcp/1.0 drift-detect' },
      });
