#!/usr/bin/env tsx
/**
 * Database builder for Ivory Coast Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Ivory Coast Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Ivorian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'CI');
    insertMeta.run('source', 'cndj.ci');
