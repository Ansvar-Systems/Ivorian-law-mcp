#!/usr/bin/env tsx
/**
 * Ivorian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://cndj.ci';
const PORTAL_NAME = 'Ivory Coast Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/ivorian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Ivorian Law MCP — Data Freshness Check');
