/**
 * get_ivorian_implementations — Find Ivorian statutes implementing a specific EU directive/regulation
...
export interface GetIvorianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface IvorianImplementationResult {
  document_id: string;
...
export async function getIvorianImplementations(
  db: InstanceType<typeof Database>,
  input: GetIvorianImplementationsInput,
): Promise<ToolResponse<IvorianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as IvorianImplementationResult[];
