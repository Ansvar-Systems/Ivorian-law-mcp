/**
 * search_eu_implementations — Search EU directives/regulations that have Ivorian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_ivorian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as ivorian_statute_count
  ...
  if (input.has_ivorian_implementation) {
    sql += ' HAVING ivorian_statute_count > 0';
  }
