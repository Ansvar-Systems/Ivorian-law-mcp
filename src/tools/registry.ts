/**
 * Tool registry for Ivory Coast Law MCP Server.
...
import { getIvorianImplementations, type GetIvorianImplementationsInput } from './get-ivorian-implementations.js';
...
export const TOOLS: Tool[] = [
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section/article) from a Ivorian statute. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Ivorian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Ivory Coast?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Ivorian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Ivorian statute aligns with or references. ' +
      'Ivory Coast is not an EU member but several Ivorian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Ivorian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_ivorian_implementations',
    description:
      'Find all Ivorian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Ivorian statutes. ' +
      'Note: Ivory Coast implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Ivorian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_ivorian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Ivorian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Ivorian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Ivorian statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_ivorian_implementations':
          result = await getIvorianImplementations(db, args as unknown as GetIvorianImplementationsInput);
          break;
